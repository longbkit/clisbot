#!/usr/bin/env node
// Stop-window write: live-active-revision-derived reply-anchor switch for the
// // `reply.anchor: thread` live-test lane. Derives from the LIVE active revision
// (never the v16 baseline) and sets route-level `reply.anchor: thread` on every
// Slack route with an agent target, so a root-level marker mints its own reply
// thread and binds at its own thread key. Telegram and the org policy stay
...
// ---- transform: slack routes -> reply.anchor=thread -------------------------
...
function transform(files) {
  ...
  for (const r of agentRoutes) r.reply = { anchor: "thread" };
  m.set(slackPath, dump(slack, { noRefs: true, lineWidth: -1 }));
  log(`slack routes set reply.anchor=thread (${agentRoutes.length} route(s))`);
...
const main = async () => {
  ...
  const revision = await store.insertManualBundleRevision({
    ...
    sourceEvidence: {
      kind: "manual",
      userId: null,
      note: "slack routes reply.anchor=thread (root marker mints dedicated thread + session)",
      at: ts(),
    },
  });
