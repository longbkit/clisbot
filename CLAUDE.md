...
| [docs/audits/pinned-vertical-contracts/](docs/audits/pinned-vertical-contracts/README.md) | Dist-verified channel-vertical contracts (entry vs plugin, `startAccount`, outbound, the inbound ctxPayload, the bundled-Telegram host monitor, loader alias routing) — one topic per file; read the matching file before writing vertical code, maintain in place                    |
| [docs/lessons/](docs/lessons/)                                                            | Dated lessons from past sessions — read the latest before a live/integration loop; `2026-08-26-integration-seams-before-live-e2e.md` is the channel-plane E2E post-mortem (spike external contracts first, real-both-sides boot test, loud failures, one live-loop script, stop rule) |
