...
import type { ChannelInteractionRenderer } from "../../channels/integration/channel-surface-contract.ts";

function normalizeBoundaryLine(line: string) {
  return line.trim().replace(/^(?::eight_spoked_asterisk:|[-*•◦·✽✶])\s+/, "");
}

export function extractRenderedIncrement(previousSnapshot: string, snapshot: string): string {
...
export function selectCompletedInteractionBody(params: {
  previousBody?: string;
  finalBody: string;
  response: "all" | "final";
}) {
  ...
  return extractRenderedIncrement(previousBody, finalBody);
}

export function mergeRenderedStreamBodies(currentBody: string, nextDelta: string) {
...
export function selectSnapshotInteractionBody(params: {
  snapshot: string;
...
function renderMarkdownRunningInteraction(body: string, note?: string) {
  if (note) {
    return body ? `${body}\n\n_${note}_` : `_${note}_`;
  }
...
function renderPlainRunningInteraction(body: string, note?: string) {
  if (note) {
    return body ? `${body}\n\n${note}` : note;
  }
...
export function renderMarkdownInteraction(params: {
  status: "queued" | "running" | "completed" | "timeout" | "detached" | "error";
  content: string;
  maxChars: number;
  ...
  if (params.status === "running") {
    return renderMarkdownRunningInteraction(body, params.note);
  }
...
export function renderPlainInteraction(params: {
  status: "queued" | "running" | "completed" | "timeout" | "detached" | "error";
  content: string;
  maxChars: number;
  queuePosition?: number;
  ...
  if (params.status === "running") {
    return renderPlainRunningInteraction(body, params.note);
  }
...
export function renderInteractionForRenderer(
  renderer: ChannelInteractionRenderer,
  params: {
    status: "queued" | "running" | "completed" | "timeout" | "detached" | "error";
    content: string;
    maxChars: number;
  ...
  return renderer === "plain"
    ? renderPlainInteraction(params)
    : renderMarkdownInteraction(params);
}

export function renderTranscriptCommandForRenderer(
  renderer: ChannelInteractionRenderer,
) {
  return renderer === "plain" ? "/transcript" : "`/transcript`";
}

export function renderChannelTranscript(params: {
  agentId: string;
  sessionName: string;
...
export const renderChannelSnapshot = renderChannelTranscript;
export const renderChannelInteraction = renderMarkdownInteraction;
