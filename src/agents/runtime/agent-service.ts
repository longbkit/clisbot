import {
  type FollowUpMode,
} from "../commands/follow-up-policy.ts";
import type { IntervalLoopStatus } from "../loops/loop-state.ts";
import {
  type RunObserver,
} from "../session/run-observation.ts";
...
import type { StoredSessionEntry } from "../session/session-store.ts";
import { SessionMapping } from "../session/session-mapping.ts";
import type { SessionRuntimeInfo } from "../session/session-runtime.ts";
import {
  type LoadedConfig,
...
} from "../routing/resolved-target.ts";
export type { AgentSessionTarget } from "../routing/resolved-target.ts";
import { TmuxClient } from "../../runners/tmux/client.ts";
import { AgentJobQueue } from "../queue/job-queue.ts";
import type { PendingQueueItem } from "../queue/job-queue.ts";
...
} from "../loops/managed-loop-controller.ts";
import {
  ManagedQueueController,
  type PromptQueueResult,
  type QueuePromptCallbacks,
} from "../queue/managed-queue-controller.ts";
import {
  RunnerService,
...
import type { StoredRecentConversationMessage } from "../routing/recent-message-context.ts";
...
export class AgentService {
  ...
  async start() {
    ...
    runCleanup();
    this.cleanupTimer = setInterval(runCleanup, cleanup.intervalMinutes * 60_000);
  }
