...
import { ClearedQueuedTaskError } from "../../agents/queue/job-queue.ts";
import {
  parseAgentCommand,
...
} from "../config/response-mode-config.ts";
import {
  getConversationAdditionalMessageMode,
  setConversationAdditionalMessageMode,
} from "../config/additional-message-mode-config.ts";
import {
  getConversationStreaming,
  setConversationStreaming,
} from "../config/streaming-config.ts";
import { setScopedConversationFollowUpMode } from "../config/follow-up-mode-config.ts";
...
import type { StoredLoopSender } from "../../agents/loops/loop-state.ts";
...
function buildLoopSurfaceBinding(identity: ChannelInteractionIdentity) {
  return buildStoredSurfaceBinding(identity);
}
      ...
      const activeText =
        error.update.status === "detached"
          ? resolveDetachedInteractionNote({
              ...
              allowTranscriptInspection: allowTranscriptInspectionForRoute(params.route),
              transcriptCommand: renderPlatformTranscriptCommand(params.identity.platform),
            })
