...
import { getConfiguredDefaultBotId } from "../../config/channels/channel-bot-records.ts";
import { DIRECT_MESSAGE_WILDCARD_ROUTE_ID } from "../../config/channels/direct-message-route-resolution.ts";
import { CHANNEL_LEGACY_CONFIG_MIGRATION_CONTRACTS } from "../integration/channel-installation-inventory.ts";
import type { ChannelLegacyConfigMigrationContract } from "./legacy-config-migration-contract.ts";
...
function migrateLegacyProviderDefaults(params: {
  contract: ChannelLegacyConfigMigrationContract;
  ...
  copyDefinedFields(params.defaults, params.legacyChannel, [...params.contract.defaultFields]);
}
...
function migrateLegacyProviderBot(params: {
  ...
  copyDefinedFields(params.bot, params.legacyChannel, [...params.contract.botFields]);
...
function migrateLegacyProviderChannel(params: {
  ...
  const defaultBotId = getConfiguredDefaultBotId({
    defaultBotId: readString(params.legacyChannel.defaultBotId) ??
      readString(defaults.defaultBotId),
    defaultAccount: readString(params.legacyChannel.defaultAccount),
    accounts: params.legacyChannel.accounts,
  });
  ...
  if (params.contract.legacyGroupKey) {
    copyLegacyRouteMap(groups, params.legacyChannel[params.contract.legacyGroupKey]);
  }
...
function migrateLegacyChannels(config: Record<string, unknown>) {
  ...
  let migrated = false;
  for (const contract of CHANNEL_LEGACY_CONFIG_MIGRATION_CONTRACTS) {
    const legacyChannel = channels[contract.legacyChannelKey ?? contract.channel];
    if (!isRecord(legacyChannel)) {
      continue;
    }
