...
function upsertSurface(
  ...
  const resolvedPlatform = surface.platform ?? platform;
  if (surface.parent) {
    ...
    directory.surfaces[surface.parent.surfaceId] = {
      ...
      platform: surface.parent.platform ?? resolvedPlatform,
      kind: surface.parent.kind,
...
function enrichSurface(
  ...
  return {
    ...surface,
    platform: surface.platform ?? record?.platform,
...
export async function recordSurfaceDirectoryIdentity(params: {
  ...
  await withDirectoryLock(pathname, async () => {
    ...
    upsertSurface(directory, params.identity.platform, context.surface, now);
