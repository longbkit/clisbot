...
import { listChannelSenderPrincipalExamples } from "../../channels/catalog/registry.ts";
import { readEditableConfig, writeEditableConfig } from "../../config/core/config-file.ts";
import type { AgentEntry, ClisbotConfig } from "../../config/core/schema.ts";
...
function renderAuthCliHelp() {
  const principalExamples = listChannelSenderPrincipalExamples();
  const principalFormatExamples = principalExamples.length > 0
    ? principalExamples.join(" or ")
    : "<platform>:<provider-user-id>";
  const primaryPrincipalExample = principalExamples[0] ?? "<platform>:<provider-user-id>";
  const secondaryPrincipalExample = principalExamples[1] ?? primaryPrincipalExample;
  ...
  return [
    ...
    `  principal format is <platform>:<provider-user-id>, for example ${principalFormatExamples}`,
    ...
    "",
    "Examples:",
    `  ${renderCliCommand(`auth add-user app --role owner --user ${primaryPrincipalExample}`)}`,
    `  ${renderCliCommand(`auth get-permissions --sender ${primaryPrincipalExample} --agent default --json`)}`,
    `  ${renderCliCommand(`auth remove-user app --role admin --user ${secondaryPrincipalExample}`)}`,
    `  ${renderCliCommand(`auth add-user agent --agent default --role admin --user ${secondaryPrincipalExample}`)}`,
