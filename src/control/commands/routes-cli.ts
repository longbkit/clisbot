...
import {
  ...
  type ParsedRouteId,
  type Provider,
} from "../../config/channels/channel-route-management.ts";
...
function validateRoutePolicy(parsed: ParsedRouteId, policy: string | undefined) {
  ...
  if (allowedPolicies.includes(policy)) {
    return policy as BotRouteConfig["policy"];
  }
  throw new Error(
...
function parseProvider(args: string[]) {
  return parseRegisteredChannelOrThrow(parseOptionValue(args, "--channel")) as Provider;
}
...
function parseOptionalProvider(args: string[]) {
  ...
  if (isRegisteredChannelId(channel)) {
    return channel as Provider;
  }
...
function parseCommandRoute(args: string[], provider: Provider) {
  return parseChannelRouteId(provider, findRouteArgument(args));
}
...
async function listRoutes(args: string[]) {
  ...
  const rows = listConfiguredRoutes(config, provider, botIdFilter);
...
export async function runRoutesCli(args: string[]) {
  ...
  if (
    ...
    const helpArgs = action === "help" ? rest : args;
    const helpChannel = parseOptionValue(helpArgs, "--channel") as MessageChannel | undefined;
    console.log(renderRoutesHelp(helpChannel));
    return;
  }
