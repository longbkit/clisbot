...
import { ProcessedEventsStore } from "../../channels/message/processed-events-store.ts";
import { loadConfig, type LoadedConfig } from "../../config/core/load-config.ts";
import {
  MissingEnvVarError,
} from "../../config/env/env-substitution.ts";
...
} from "../commands/operator-errors.ts";
...
} from "../../channels/integration/channel-plugin.ts";
...
export class RuntimeSupervisor {
  ...
  private async createRuntime(
    ...
    const startedChannels = new Set<ChannelPlugin["id"]>();
    let startupPhase: "agent" | "channel" = "agent";
    let startupChannelId: ChannelPlugin["id"] | undefined;
    ...
    try {
      ...
      if (startupPhase === "channel" && startupChannelId && !startedChannels.has(startupChannelId)) {
        const plugin = this.dependencies.listChannelPlugins()
          .find((entry) => entry.id === startupChannelId);
        if (plugin?.describeStartupFailure) {
          await this.dependencies.runtimeHealthStore.markFailure({
            channel: startupChannelId,
            error,
            diagnostic: plugin.describeStartupFailure(error),
          });
        }
      }
