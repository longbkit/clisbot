...
export function renderLoopsHelp() {
  return [
    ...
    "  - `--sender <principal>` is required when creating loops, using `slack:<user-id>`, `telegram:<user-id>`, or `zalo-bot:<user-id>`",
...
export function renderLoopsCreateHelp() {
  return [
    ...
    "",
    "Usage:",
    `  ${renderCliCommand("loops create --channel <slack|telegram|zalo-bot> --target <surface> --sender <principal> <expression>")}`,
    `  ${renderCliCommand("loops --channel <slack|telegram|zalo-bot> --target <surface> --sender <principal> <expression>")}`,
    "",
    "Required:",
    "  - `--channel <slack|telegram|zalo-bot>` and `--target <surface>` select the routed session",
    "  - `--sender <principal>` records the human creator, for example `slack:U1234567890`, `telegram:1276408333`, or `zalo-bot:user-123`",
    "",
    "Optional:",
