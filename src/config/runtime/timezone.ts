...
import { listChannelManagedProviderDefaults } from "../channels/channel-bot-contract.ts";
...
export function resolveConfigTimezone(params: {
  ...
  const agentTimezone = params.agentId
    ...
    : undefined;
  const legacyProviderDefaults = listChannelManagedProviderDefaults(params.config).map(({ defaults }) => ({
    source: "legacy-provider-default" as const,
