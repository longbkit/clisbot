...
function getProviderBot(
  ...
  provider: Provider,
  botId: string,
): SlackBotConfig | TelegramBotConfig | ZaloBotConfig {
  const providerBots = provider === "slack"
    ? config.bots.slack
    : provider === "telegram"
  ...
  if (!bot) {
    const providerLabel = provider === "slack"
      ? "Slack"
      : provider === "telegram"
        ? "Telegram"
        : "Zalo Bot";
    throw new Error(`Unknown ${providerLabel} bot: ${botId}`);
  }
  return bot;
}
