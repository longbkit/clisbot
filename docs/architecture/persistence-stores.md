...
| Runtime monitor state | `<state>/clisbot-monitor.json` | `src/control/runtime` | Monitor pid, worker pid, start/update timestamps, backoff/restart state | Canonical for monitor coordination | Single monitor invariant; direct full-file write; lifecycle readers fence persisted pids with OS process role and start time | Rewritten by monitor lifecycle | Keep the single-writer invariant explicit. A numeric pid is never sufficient process identity when persisted monitor state exists. If other processes start mutating this file, move to `json-storage` or an owner-process command path. |
