...
- Mention-gated routes still let shared command-like messages reach the shared interaction processor. Cover `/queue`, `\q`, configured slash shortcuts, and bash shortcuts through the shared agent-command helper instead of provider-local prefix lists.
