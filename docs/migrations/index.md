...
```text
Path: 0.1.52 -> 0.1.53-beta.5
Update path: direct
Manual action: none
Risk: medium
Automatic config update: no new schema migration in `0.1.53-beta.5`; existing automatic config updates from earlier versions still apply when upgrading older installs
Breaking change: no
...
`0.1.53-beta.5` is a broad internal boundary refactor beta. It does not require
...
restart or reload. Beta testers should still watch startup, route, queue, loop,
