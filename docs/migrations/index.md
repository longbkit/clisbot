...
```text
Path: 0.1.52 -> 0.1.53-beta.3
Update path: direct
Manual action: none
Risk: medium
Automatic config update: no new schema migration in `0.1.53-beta.3`; existing automatic config updates from earlier versions still apply when upgrading older installs
Breaking change: no
...
`0.1.53-beta.3` is a broad internal boundary refactor beta. It does not require
...
release/publish recovery text from update help. Beta testers should still watch
startup, route, queue, loop, and channel-specific behavior because many files
moved behind the same public contracts.
