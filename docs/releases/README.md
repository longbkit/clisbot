...
Release notes answer: what changed in this version, what user or operator impact it has, and which upgrade or validation facts matter later.

Keep narrative catch-up material in [`docs/updates/`](../updates/README.md). Keep concrete upgrade procedures in [`docs/migrations/`](../migrations/README.md).
...
4. `docs/migrations/vA.B.C-to-vX.Y.Z.md`: only when config, schema, or runtime upgrade steps need a concrete runbook.
...
- Create a migration note for a beta only when beta testers need a concrete upgrade or rollback runbook.
