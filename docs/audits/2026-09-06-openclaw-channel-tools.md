...
| Durable inbound sau restart                      | Core `src/channels/message/ingress-queue.ts`, `ingress-drain.ts`, `src/plugin-sdk/channel-ingress-runtime.ts`; Zalo `extensions/zalo/src/webhook-spool.ts` từ `0ac69b9fe80`, durable webhook acceptance ở `601a405430d`                           | **Mới chỉ có một phần**: Fusion `shared/monitor.ts` ghi ledger trước handoff, dedupe replay và giữ polling; chưa có persistent pending queue, claim lease, per-conversation lane, retry/dead-letter, restart drain hoặc explicit durable ACK header. `delivery_ledger` có `failed/attempts` nhưng semantics đó đang áp dụng cho `out`; inbound chỉ có `recorded/consumed`, không lưu payload để retry. |
