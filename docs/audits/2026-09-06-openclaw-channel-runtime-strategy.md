...
Date: 2026-09-06. Mode: architect / implementation baseline. Progress ledger: [2026-09-07 goal ledger](2026-09-07-openclaw-channel-port-goal.md). Status: the strategy below (§0 onward) remains the source of truth for ownership, source fidelity and delivery posture. The implementation checkpoint is a dated snapshot, kept as history.
...
Ingress disposition and drain lifecycle now follow upstream policy. `src/channels/message/ingress-retry-policy.ts` is ported verbatim into `@getpaseo/channels-core` with its `computeBackoff` dependency, so Fusion and OpenClaw share one backoff schedule, attempt ceiling, dead-letter minimum age and non-retryable hook; the Hub supplies the Hub-owned non-retryable classes (`packages/hub/src/channels/ingress/non-retryable.ts`). The supervisor owns one drain per account (`packages/hub/src/channels/ingress/drain.ts`): a pass on start, a timer for retry-due and recovered rows, an admission wake so a live message does not wait for the tick, lease refresh while a dispatch runs, and a `stop()` that clears every timer. `ChannelStore` carries the operator verbs — `resubmitChannelIngress` (dead-letter → pending with a fresh budget) and `pruneChannelIngress` (completed / dead-lettered retention). Store and drain are tested against a migrated embedded database, including a crashed-worker claim recovered and delivered exactly once and a second claim refused while the first lane lease is live.

Upstream's own `ingress-queue.ts` / `ingress-drain.ts` are not portable: both are written against a single-process SQLite file (Kysely writers, `blockedLaneKeys` computed in memory, `pid@boot-id` claim ownership, OpenClaw gateway work admission). Fusion serializes lanes inside the claim statement instead. Every omission and its coupling is recorded in `packages/channels/core/upstream-sync.json`.

Streaming and progress now have a producer on the Hub side. `packages/hub/src/channels/streaming/` turns the agent turn's accumulating assistant text into a live draft: the first delta opens it, later deltas extend it at the channel's pacing, and the relay's final post finishes the draft in place. Which transport carries it is decided by feature detection on the loaded vertical's `plugin.outbound` — upstream's `startSlackStream`/`appendSlackStream`/`stopSlackStream` for a native Slack stream, otherwise the `updateText` drive verb both verticals publish, which is what makes Telegram stream without a Telegram branch in the Hub. A route opts in through the new `sync.streaming` knob (upstream's `off`/`partial`/`block`/`progress` plus `nativeTransport`); the knob is absent at the org floor, so every revision authored before it keeps posting one final answer. Every degradation ends the same way: the draft breaks, the finalize transport falls back to the plain post path itself, and the reply is delivered once on one delivery-ledger row.

The ported native primitives still take a live platform client — `startSlackStream` a `WebClient`, `createTelegramDraftStream` a grammY `Bot["api"]` — which the Hub does not hold. Until the verticals publish a Hub-drivable start (the `outbound-adapter` port slice 21 left open), the real accounts stream through the edit-in-place draft and the progress card, and the native path is exercised by the producer's fake-plugin tests.

This checkpoint was partial, and much of what it listed as missing has since landed: the Discord, Google Chat, Feishu, Zalo and Zalo Personal verticals ship with their Hub wiring, message actions and presentation are on the production path, the source manifest replaced the hand-written tables, the setup UI exists, and resubmit/prune have an operator surface. Read the goal ledger for what each slice actually closed.
...
| Dễ sync là thực tế                | Đã đo 2026-09-07 (`apply --from/--to` trên bản sao cô lập, dải `e54cb3cb857` → `5d8067a4483`, một tuần upstream): 120 file `verbatim` merge sạch, 0 conflict, 118 file trùng byte với bản port tay; manual adaptation còn 2 dòng import và 15 file upstream mới trong dải (~40 phút), typecheck và test của cả năm package xanh. Vướng mắc còn lại: 50 file `adapted` có thay đổi đều conflict (core 45), upstream thêm 13 test file trong dải mà bản port thiếu 12 (test nằm ngoài manifest nên `apply` không thấy); một upstream file bị `git merge-file` coi là binary, và mỗi package chỉ có một `baselineCommit` nên sync dở dang làm `check` fail; runbook: [Following a delta with `apply`](../guides/developer-guide/upstream-sync-and-contribution.md#following-a-delta-with-apply) |
...
Kế hoạch sau merge phải tiếp tục ba workstream cụ thể: (a) package/setup/dependency manifest cho channel plugin mới như Signal, Mattermost, IRC, SMS và A2A; (b) hoàn thiện durable ingress queue/drain theo policy upstream, operator resubmit/prune và Zalo webhook-spool semantics; (c) message-tool/presentation/action/extra-tool adoption cùng receipts và callback. Chỉ sau khi source mapping và tests của từng workstream được port mới tính capability là Available.

Đặc biệt, A2A có agent card unauthenticated, peer Bearer authorization, HTTP JSON-RPC, `SendMessage`/`GetTask`, rate limit và task ownership. Nó cần security/config/setup/retention review riêng; không đưa vào một bảng “chat channel” bằng cách đổi tên target. Core ingress queue của OpenClaw dùng SQLite claim/drain; Fusion hiện đã có queue contract Hub-owned tương đương cho admission/claim/lease/lane, nhưng vẫn phải port policy/lifecycle còn thiếu và không được đánh dấu đã khớp chỉ từ tên interface.
