...
Date: 2026-08-31. Status: architecture proposal; no product code in this document is implemented
unless a section is explicitly marked **CURRENT**. Scope: put Hub account and management surfaces
in the shared Paseo app, let an authenticated user discover and open the daemon Projects they may
use, and preserve ordinary Paseo client/daemon compatibility when managed access is disabled.
...
| ------------------------ | -------------------------------------- | ---------- | --------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| `projects:read`          | List legacy Hub deployment Projects.   | Hub API    | `CURRENT` | Keep for older API compatibility; new authoring lists Triggers through `configuration:validate`. Do not reuse as Member Project access. |
| `configuration:validate` | Validate a complete Hub configuration. | Hub API    | `CURRENT` | Keep. Paseo uses session-authenticated operations instead of an API key.                                                                |
| `configuration:install`  | Install and activate a configuration.  | Hub API    | `CURRENT` | Keep. Paseo activation applies the equivalent Member authorization separately.                                                          |
| `runs:dispatch`          | Dispatch a manual workflow run.        | Hub API    | `CURRENT` | Keep distinct from Member `automation.run`.                                                                                             |
...
| Hub Trigger/configuration and Channel policy                              | Organization-owned revisioned Triggers, durable execution, legacy multi-step compatibility, Channel routing, multi-account concepts, roles/privileges, and approval policy experiments. | Reuse the Trigger backend and privilege algebra. Move unreleased Channel authoring out of the legacy Project instead of preserving that container.                         |
...
| Hidden Hub Trigger runtime Project                       | Adapts one organization Trigger revision to the current durable workflow engine. | Never expose it as a product resource or call it a Paseo Project. Remove the adapter when the execution engine accepts Trigger revisions directly. |
| Dashboard server functions                               | Couple the current React dashboard to Hub operations.                            | Expose only the stable JSON operations the shared app needs; no obligation to retain the server-function boundary.                                 |
...
- organization Trigger revisions, legacy-workflow migration, and durable execution;
- multi-account channel routing; and
...
Sign in to Hub
  -> Add Channel account
  -> choose Conversation and Audience
  -> choose a fixed Automation, or Project + Agent controls
  -> optionally Apply Agent profile
  -> choose the Route-bound Channel reply actions and output roots
  -> validate, activate, and test the Route
  -> Open in Paseo
  -> obtain accessTicket
...
| `packages/hub/src/channels/config/schema.ts`                    | `OutboundDefaultsSchema`, `RouteSchema` (**CURRENT**); `ChannelActionPolicySchema` (**NEW**)                                 | Add the exact Route-bound reply-action ceiling and Project/output roots; preserve existing `outbound.path` input and defaults.                                                                   |
...
| `packages/hub/src/channels/policy.ts`                           | `resolvePrincipal()`, `privilegeHolds()`, `effectivePrivileges()`, `classifyToolClass()` (**CURRENT**)                       | Keep Channel identity/scope adaptation and `approval.channel` classification here; do not interpret a tool name as execution authority.                                                          |
| `packages/hub/src/channels/bindings/index.ts`                   | `BindingEngine.bindOrSteer()`, `.createAgent()` (**CURRENT**)                                                                | Pass the existing durable thread-binding ID and compiled Route action ceiling into Agent creation; invalidate the capability with the binding/Route revision.                                    |
...
| `packages/hub/src/channels/control-plane.ts`                    | `createChannelAgentSpecResolver()` (**CURRENT**)                                                                             | Attach the Channel broker using the opaque binding capability and preapprove only actions compiled for that Route.                                                                               |
...
| `packages/app/src/screens/settings-screen.tsx`                   | `SettingsSidebar`, `SettingsScreen` (**CURRENT**)                                                                                         | Show “Sign in to Hub” while signed out and Account/Managed Hosts/Team & Access entries while signed in.                                                                                           |
