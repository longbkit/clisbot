...
**In-repo route (2026-08-26 pull):** the layout above is the **pinned route**. For `loadMode: "in-repo"` (Slack, Telegram) the install skips the supply path entirely — no tarball fetch, no integrity gate, no `openclaw@…/` main-dir provisioning; the Hub drives its own workspace package (`inRepoPackage`, resolved from the monorepo; a missing built entry module fails closed, refusing the install). The account root still gets its per-channel marker (`install-<channel>.lock`, `loadMode: "in-repo"` + `inRepoPackageDir` + the sync-reference pin recorded for the re-sync loop).
