...
#    -> the starter agent runs on a provider the daemon must be able to execute. Before wiring
#       a channel route's `agent:` target, list what is valid on this daemon:
#         clisbot provider ls                   # providers + status (available/unavailable) + enabled + modes
#         clisbot provider models <provider>    # the valid model ids for that provider
#         clisbot provider diagnostic <provider># installation / environment / availability detail
#       the revision validator rejects a route whose provider is not `available` (or whose model/mode
#       is not in those lists), so list first, then pick. Non-default daemon port: add --host <host:port>.
...
That single line does, in order: (1) start the daemon if not running (`daemon start` pattern, `$CLISBOT_HOME`); (2) start the embedded Hub if not running (`hub start` pattern, §3.2); (3) run the starter `hub init` config + deploy if the Hub has no active revision yet; (4) create the **bot** (below); (5) verify the channel plane booted (the Hub's `channels/status` reports the account's transport) and print the one-screen state: daemon + hub host/pid, the account's transport, the agent + workspace, and the "mention the bot" next step. Idempotent: re-running with the same flags reuses the existing bot; only a changed flag (new provider, new `--bot-type`) re-creates.
...
| `packages/hub/src/channels/install/**`           | Shared channel package install: reads `channel-pins.json`, fetches pinned tarballs from public npm, verifies `dist.integrity`, and installs package code under `<CLISBOT_HUB_DATA_DIR>/plugins/channels/` (default `~/.clisbot`); account directories contain runtime state only                      |
...
- **In-repo route (2026-08-26 pull):** Slack/Telegram do **not** use the alias hooks above — their packages carry zero `openclaw/*` specifiers, so no alias-route applies; the loader admits the in-repo package dirs as load-trace allowlist roots and the drive surface is asserted by the contract test instead ([2026-08-26-in-repo-channel-verticals.md](2026-08-26-in-repo-channel-verticals.md) §6.5 rule 1–2, §4.5). The alias seam + `hosts/` bound modules stay live for pinned channels only (zalouser + later). The pre-pull `hosts/telegram-monitor.ts` option-A override is no longer installed (supervisor comment at `supervisor/index.ts`: the in-repo L2 owns the poll loop) — its contract record remains in §4.8 / [`telegram-monitor.md`](pinned-vertical-contracts/telegram-monitor.md) for the re-sync loop.
...
CLISBOT_HUB_DATA_DIR/          # default ~/.clisbot/hub (inside the machine home; §4.5)
  ...
  plugins/
    channels/                  # the shared managed npm-style project (one per Hub)
      package.json             # name clisbot-channel-plugins, private, deps = pinned packages
      package-lock.json        # lockfileVersion 3: version + integrity + resolved url
...
**Managed published/bundled route (current):** when a channel uses pinned npm
supply, package code is installed once under
`CLISBOT_HOME/plugins/channels/` with a package manager `package.json`,
`package-lock.json`, and `node_modules/`. The lockfile replaces the former
`install-<channel>.lock` markers, and account directories contain runtime state
only.
...
defaults:
  ...
  reply: { anchor: default }   # default: the reply lands where the marker came from (thread → thread, root → root)
  sync:
    finalAnswers: true
    progress:                       # "the bot is working" — three surfaces, folded per leaf
      progressMessage: true         # the throttled relay line
      typingIndicator: true         # the provider's native typing status
      messageReaction: off          # off | an emoji name: the receipt on the sender's message
    toolCalls: false
...
transport: # channel-native block, passed to the pinned vertical
  ...
  inlineButtons: dm # where native approval cards may appear (P0 from 2026-08-27); `off` = text + command prompt
...
defaults: # account baseline for every route; inherits the
  ...
  interaction: # how the bot engages
    ...
    followUp:
      mode:
        mention-only # mention-only (default): every message must mention the bot;
        #   auto: unmentioned follow-ups in a bound conversation keep
        #   steering the same session until it idles out
      ttlMinutes: 5 # auto only: "idles out" = no agent activity for this long (and after a Hub restart)
  binding:
...
routes: # ORDERED; first match wins
  ...
  - match: { kind: channel, ids: [C0INFRA] }
    ...
    reply: { anchor: default } # ... and the reply follows the marker's level (root → root, thread → thread)
...
**Decision (2026-08-27): the native approval card is P0, not P0.5.** The two-surface approval goal — a pending tool permission is answerable from the channel thread AND from any paired Paseo client, exactly-once — pulls the card into P0: `inlineButtons` places the card as of P0, and the text + command prompt remains where `inlineButtons` excludes the surface. The card extends the drive surface both ways; the in-repo verticals own it:

- **Outbound:** a rich post (Slack block-kit card, Telegram inline keyboard) and an in-place **message update** so a card answered on ANY surface updates where it was posted instead of staying stale.
- **Inbound:** button callbacks arrive as transport events — Telegram `callback_query` inside the existing `getUpdates` poll, Slack interactive events over the existing Socket Mode connection. The button carries no authority: the answer runs the same `answerFromChannel` second authority check as the typed command, so a card button is as inert as a typed `approve <id>` for a responder without the class privilege.
- **Zero daemon diff is unchanged.** The daemon still sees exactly one `agent_permission_response` from the Hub's trusted session, no matter which surface answered; the card is Hub + vertical code only.

The acceptance this adds (fast loop + a live sample): permission rules (auto-allow / auto-deny / prompt, per tool class), streaming options (`sync` progress / toolCalls / finalAnswers on–off), approval (two-surface answer, race, wrong-approver inert), follow-up (resume, not re-create) — on both surfaces per channel: Telegram basic group **and** forum topic (`message_thread_id`), Slack channel root **and** thread reply.
...
**`reply.anchor` — where the bot's outbound posts land.** The inbound side (which session a message reaches) and the outbound side (where replies are posted) are independent, and channel-generic in the same sense: both values follow the marker — the native thread the message arrived in, else the conversation root — and `thread` adds one step: a root-level marker opens the reply as a thread of its own, by answering the marker message itself (Slack: `thread_ts` = the marker's `ts`; Slack has no create-thread API; OpenClaw's `replyToMode: all`):

| `reply.anchor` | Outbound posts go to                                                                                                                               |
| -------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| `default`      | the marker's own level: a thread marker replies in that thread, a root-level marker replies at the conversation root (OpenClaw `replyToMode: off`) |
| `thread`       | the marker's own level, except a root-level Slack marker mints the reply thread on the marker message itself                                       |

Minting is per-turn and marker-keyed: it needs the live marker's native message id, so it happens only on Slack channels (not DMs — no thread level — not Telegram, and not after a restart re-attach, which carries no live marker and falls back to the binding's persisted thread). The binding's persisted thread always wins over the live marker (the `binding.key: thread` case).

Four working configurations, each a `defaults:` block:

| `binding.key` | `reply.anchor` | Effect                                                                                                                                              |
| ------------- | -------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| `thread`      | `default`      | **Default.** One session per thread/topic; replies stay in the thread. The Telegram-group-with-topics / Slack-thread use case.                      |
| `channel`     | `default`      | **One session per whole channel**; every thread in it (and channel-level messages) is one continuous session; the reply follows the marker's level. |
| `channel`     | `thread`       | One session per channel; thread markers reply in their thread, and each root-level Slack marker mints its own thread on its message.                |
| `thread`      | `thread`       | One session per thread/topic; replies stay in the thread, and a root-level Slack marker mints a new thread on its message.                          |

Worked example — Slack `#general` where one long-running session covers the whole channel and the reply lands where the question was asked:

```yaml
# channels/slack/general.yml
defaults:
  binding: { key: channel }
  reply: { anchor: default }
```

Worked example — Slack `#infra` where the operator wants one long-running session for the whole channel and every root-level question gets its own thread:
...
`channels/telegram/support.yml` — **Telegram, polling, multi-surface** (forum topics = their own sessions; the reply follows the marker's level):
...
defaults:
  ...
  reply: { anchor: default } # ... and the reply follows the marker: in-topic → in-topic, root → group root
routes:
...
| `defaults.interaction.followUp.mode`                      | `auto` \| `mention-only` (default `mention-only`) | org default follow-up behavior (§4.3.4)                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `defaults.interaction.followUp.ttlMinutes`                | int (default `5`, clisbot `participationTtlMin`)  | org default idle window                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `defaults.binding.key`                                    | `thread` \| `channel` \| `dm` (default `thread`)  | org default session granularity (§4.3.4)                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `defaults.reply.anchor`                                   | `default` \| `thread` (default `default`)         | org default reply location (§4.3.4)                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `defaults.sync.finalAnswers/toolCalls`                    | bool                                              | which timeline slices relay into the thread (the ROOT agent's own timeline)                                                                                                                                                                                                                                                                                                                                                                                      |
| `defaults.sync.progress.progressMessage`                  | bool (default `true`)                             | the throttled progress line relayed into the thread (the old bare `sync.progress` boolean means exactly this, and only this)                                                                                                                                                                                                                                                                                                                                     |
| `defaults.sync.progress.typingIndicator`                  | bool (default `true`)                             | the provider's native typing status while the turn runs, raised when the plane ACCEPTS the inbound (not on a daemon turn event). Slack `assistant.threads.setStatus` needs a `thread_ts` and anchors on the reply thread, else on the sender's own message `ts` — an unthreaded ask still shows it; `chat:write` serves the method, `assistant:write` is the soft compatibility scope. Telegram `sendChatAction`, re-sent by the vertical on its own 4.5 s timer |
| `defaults.sync.progress.messageReaction`                  | `off` \| emoji name (default `off`)               | react to the SENDER's own message for the length of the turn — the receipt it was taken; removed on close. Slack only (the Bot API has none); [`typing.md`](pinned-vertical-contracts/typing.md)                                                                                                                                                                                                                                                                 |
...
| `defaults.sync.subagents.finalAnswers/progress/toolCalls` | bool (all default `false`)                        | the same three relay knobs scoped to subagent (Task tool) text, which rides the `agent.provider_subagents.update` wire frame (the `provider_subagents` client capability); posted into the original thread with a `▶ {label} (subagent): ` prefix; root `sync.*` gates only the root timeline                                                                                                                                                                    |
...
| `transport` — `inlineButtons`              | `off` \| `dm` \| `group` \| `all` \| `allowlist`    | where native approval cards may appear (P0 from 2026-08-27); `off` keeps the text + command prompt               |
...
| `defaults.reply.anchor`                    | `default` \| `thread`                               | where the bot's outbound posts land (§4.3.4)                                                                     |
...
**Daemon auth for protected homes.** A dev home protects its daemon with a password file, `<home>/.daemon-password` — a single `PASEO_PASSWORD=<value>` line (a bare value is accepted too). `hub start` and `bot start` read that file at spawn time and pass the value to the Hub as `PASEO_PASSWORD`: without it, the Hub's daemon link is rejected against a password-protected daemon. Only the home's file is read — never the repo's `.env`. The CLI's own daemon connections in the same flow (workspace + agent creation in `bot start`) carry the same value through the `tcp://host?password=…` connection URI, so a protected home works end to end.
...
11. **The pinned main tarball ships no `node_modules`** — verified against the integrity-matched registry bytes, it ships declared deps + a resolved `npm-shrinkwrap.json`, and `npm ci` on it crashes arborist on this lockfile's peer-set shape. The install plane provisions the main dir's production closure from the shipped shrinkwrap (fetch → verify against the lockfile-recorded integrity → extract), while the managed root `package-lock.json` records the installed package metadata. The Slack tarball shape (ships its own tree) is left alone. Contract + verification: `docs/audits/pinned-vertical-contracts/install-supply.md`.
12. **`supervisor/index.ts` exceeds the 700-line hard limit — provenance, not the card slice.** The file was already over the limit in the P0 wave (HEAD ~848 lines); the 2026-08-27 native approval-card slice (E1/E2/E3/E5) added only `updateFor`, the `channelRuntime.approvalAction` mount, and the subagent-frame path (~119 lines → 967). A documented-exception comment now sits at the top of the file; the split decision (extract `createChannelPlane` wiring + the lifecycle loop into `supervisor/plane.ts` / `supervisor/lifecycle.ts`) is left to the next hub-integration pass rather than done mid-slice.
13. **The live Hub runs the Vite bundle, not `dist/` — so two builds coexist in one running process.** `bin/paseo-hub.js` imports `dist/index.js`, whose `loadBuiltStartServer()` loads `.output/server/start-server.js` (the Vite build). The supervisor / plane / daemon-client code that runs live is therefore the `build:hub` (Vite) output, while the loader's host modules (seam, runtime store) are served from tsgo `dist/` by default (`hostBaseDir`) — bridged by the `Symbol.for("@getpaseo/hub/channel-runtimes")` store. Consequences: `build:node` (tsgo) refreshes the host modules but not the live runtime; a runtime-code fix reaches the live Hub only through `build:hub`, and the Vite build OOMs while the Hub is resident, so `scripts/e2e-dev.sh build` stops it first. "My new log line did not appear" → grep the string in `.output/server/assets/*.js`, not `dist/`.
14. **Provider availability is a two-stage gate on the daemon, and the channel revision validator enforces the second.** A provider is _configured_ only when the daemon's registry has a definition for its id: the built-in ids (`claude`, `codex`, `copilot`, `cursor`, `opencode`, `pi`, `omp` — `BUILTIN_PROVIDER_IDS`, `packages/protocol/src/provider-config.ts`) plus **custom overrides persisted under `agents.providers` in `<home>/config.json`** (`ProviderOverrideSchema`: `extends`, `label`, `command`, `env`, `models`, …; a custom id must declare `extends`, e.g. `extends: "acp"` + `command: ["grok","agent","stdio"]`). A fresh home has no `agents.providers` key, so only built-ins exist there — `list_provider_models` / `validateAgentConfiguration` for any other id answer `Unknown provider` / `is not configured`. The second stage is _enabled + status ready_ (`provider-snapshot-manager.ts`: a disabled or not-ready entry also fails validation). Toggling a provider from the app/CLI is a `set_daemon_config` write: the config store persists it into `<home>/config.json` **and hot-applies it** (`applyMutableProviderConfig` → registry rebuild) — no daemon restart. The channel-plane revision writer live-validates every route's `agent:` against the daemon before committing (`validateAgentConfiguration` mirror in `.hub-revision-write.mjs`), so a provider not toggled on that daemon blocks the route: "the binary is installed on the machine" satisfies stage 0 only. Operator surface: `clisbot provider ls | models <p> | diagnostic <p>` (daemon RPCs `get_providers_snapshot` / `list_provider_models` / `provider_diagnostic`; non-default port: `--host <host:port>`).

15. **Provider enablement onboarding: keep the built-in pre-enable model; new-provider definitions and PATH-discovery are upstream (out of P0).** Problem: a channel route's `agent:` provider must exist **and** be enabled on the daemon, and an operator needs a principled onboarding path for a provider that is not a built-in (e.g. the `grok` ACP provider) — not a hand-run script. The daemon's existing model: built-in ids are pre-enabled by default (`enabled: override?.enabled ?? definition.enabledByDefault ?? true`, `provider-registry.ts:727` — `omp` opts out via `enabledByDefault:false`; claude/codex/copilot/opencode/pi are on), and a pre-enabled provider whose binary/auth is missing probes to `status:"unavailable"` (`refreshProvider`, `provider-snapshot-manager.ts:951`) — shown as "unavailable," not an error, and not blocking. That is **safe and cheap**: the availability probe (`isAvailable`) is a binary/launch check, and the expensive catalog/model fetch runs only for available providers, so a not-installed provider costs a fast `which`-style miss. **Decision:** (a) keep the built-in pre-enable model as-is — it already answers "pre-enable the popular remaining ones": **pi and opencode already are** built-in + pre-enabled; **grok is not a built-in**, so "pre-enable grok" = "ship it as a known provider definition," which is an upstream `packages/protocol`+`packages/server` product decision (add to `AGENT_PROVIDER_DEFINITIONS`/`BUILTIN_PROVIDER_IDS`, flag-gated), out of P0 scope (zero daemon diff). (b) **PATH-discovery (scan the machine, auto-enable found providers)** is the better long-term UX but a new daemon behavior (startup scan + auto-create-override) → also upstream/P1; caveats: a binary on PATH is not authed/usable, discovery must be a cheap, idempotent probe feeding the same snapshot manager, not bypassing it. (c) **P0-correct onboarding for a custom/ACP provider:** the operator configures + enables it on the daemon via the app provider toggle or a `set_daemon_config` write (persisted + hot-applied, note 14); the channel revision validator live-validates and reports "provider not available on this daemon" when it is not. (d) **The P0-scoped onboarding gap (recommended follow-up, flag-gated, `packages/cli` only):** `clisbot provider` is read-only today (`ls`/`models`/`diagnostic`); add a write verb (`provider add`/`enable`) so an operator has a clean CLI path — recorded, not built in this wave. **`node .set-dev-grok.mjs apply` is a dev-daemon test fixture** (the H5 E2E unblock — the same override state an operator had manually created on the production daemon), **not a user onboarding path**; do not present it as one.
...
- **`openKeyedStore` is a per-account in-Hub store, not OpenClaw's SQLite.** The pinned verticals use `state.openKeyedStore` / `openSyncKeyedStore` for durable per-key state (Telegram's long-poll offset, Slack's send-dedupe cache). The P0 host runtime supplies a real stateful store backed by JSON files in the account's channel state dir (`channels/<organizationId>/<channel>/<accountId>/state/`), so restarts keep the poll offset and the dedupe cache — but the OpenClaw SQLite-backed contract (row limits, TTL eviction semantics, `overflowPolicy`) is approximated, not implemented. The P0 verticals only call `register` / `lookup` / `registerIfAbsent` / `consume` / `clear`; a vertical that depends on eviction limits or TTL expiry is out of P0 scope. The Telegram host monitor (§4.8 D4) owns its poll offset through this store; the native SQLite-backed ingress queue (`openChannelIngressQueue`, `$OPENCLAW_STATE_DIR/state/openclaw.sqlite`) never loads at P0, so **no SQLite file is created by the P0 host** — persistence is JSON keyed-store files under the account's CLISBOT_HOME state path.
...
**Post-pull scope (2026-08-26):** for Slack and Telegram the pinned dists are now the **upstream sync reference** only — the bytes each in-repo package's `SYNC.md`/`DEVIATIONS.md` ported from — not the live supply (`loadMode: "in-repo"`, [2026-08-26-in-repo-channel-verticals.md](2026-08-26-in-repo-channel-verticals.md) §6.5). The drive-surface contract (B3) is unchanged and asserted against the in-repo packages by the extended contract test; the live behavior of a pulled channel lives in `packages/channels/*`, and these dist files stay for the re-sync loop. For zalouser and later pinned channels the folder remains the live-supply contract.
...
## 5. Testing the channel plane (live scenarios)

The scenario catalog lives in [docs/tests/channels/p0-live-scenarios.md](../tests/channels/p0-live-scenarios.md) — one file, grouped A–F (inbound gate, thread binding, outbound relay, config, native capability, CLI/lifecycle/flags), each scenario with numbered assertions, an honest status legend, and a findings register. Test strictly against that catalog: a scenario is done when every assertion has live evidence; never report a test as finished from static inspection, and never mark `PASS` for a chain that only partially fired.

Three expectations define what the outbound side must do:

1. **Stream → filter → post by config.** The agent's timeline is the source; the sync-policy filter selects which events cross; the post lands per `binding.key` + `reply.anchor` (the marker's native thread, the conversation root, or — under `thread` — a new thread minted on the marker message). Verified shape: the relay records the delivery before posting, and the final answer is the turn's last relay post.
2. **Format conversion at the boundary.** Agent answers carry their own format (markdown, code blocks). Before posting, convert to the channel-native form (Slack mrkdwn / Block Kit, Telegram HTML) so nothing leaks raw and nothing breaks channel entities. OpenClaw's verticals already carry this kind of conversion — reuse it, do not write a new converter. Currently both in-repo verticals expose `outbound.sendText` (plain text) only, so this expectation is the gap the card/rich slice fills.
3. **Native capability via a Hub-attached tool.** When the answer needs a native surface (e.g. Slack chart blocks, interactive cards), it goes through a dedicated MCP tool the Hub attaches to the agent — an extension of how the Hub drives the agent today, not ambient agent-environment tooling. **Open design decision (2026-08-27, undecided):** when an outcome can be delivered both through that tool and through the streamed final answer, which path wins and how do the two stay consistent? Do not implement either way before the real behavior is observed (scenario E3 in the catalog is the probe).
4. **Media, two-way, one or many.** Inbound: images, files, audio, voice notes, and video attached by an external sender must reach the bound agent (downloaded under the shared home, content handed to the agent — captions included; media-only messages need an explicit admit/ignore decision). Outbound media is explicit: the Hub-attached `send_file` MCP tool validates an absolute path under `homeRoot`, applies the channel size policy, records before posting, and drives the vertical's native surface (Slack file post / Telegram `sendPhoto|Document|Audio|Voice|Video`). This follows OpenClaw's structured media payload parts. The relay posts final answers as plain text; heuristic path extraction is retired.

Status discipline: statuses in the catalog rot with every code change and every Hub restart; the acceptance mapping at the bottom of the catalog is the single source for what "P0 acceptance" (plan §10) actually requires, and the findings register there names every known blocker (as of 2026-08-27: `hub stop` not terminating the live Hub; the Telegram L1 message-id space not matching the test group's real ids; ambient agent-environment Slack tooling polluting channel sessions).
