...
| A session title too long for its line                   | **Full titles** wraps it. With it off, hover the cut title to read it. |
...
Everything lives on one page, **Show → Agent sessions** — the first row of the Show page, in a group of its own — in the sidebar's display preferences, in four groups split by separators: the switch, expansion, Active sessions only with Full titles, and the details a line shows. There is no deeper submenu.
...
User, channel, Updated time and the metadata status only appear on hosts that can read session storage. Model, Created time and Last activity come from the agent record and show on any host. The items are drawn by the same component as the workspace row (`SessionMetadataLine`), so they look and behave the same. A user name and a channel there are text, not controls: in the sidebar the row owns the press. The profile is a press away from the person's message inside the session; the channel's ids, which the workspace row used to show in a menu, are not shown in the sidebar at all.
...
- **Fusion defaults differ from upstream (decided 2026-09-16, before rollout, so no migration).** Show sessions is on, with Model, Created user and Channels on a session line. The workspace row also shows Branch and Created user, and its trailing slot shows Last activity instead of Diff stats (`display-preferences/row-items.ts`, `hooks/use-settings/storage.ts`). A channel-driven sidebar is read by who, where and when; upstream's defaults suit a single developer's branches. Settings are saved in full on first load, so a later default change reaches only new installs.
