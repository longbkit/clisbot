...
(1,024 events / 8 MiB per agent) until they are written or released. The manager waits
for each event's row to be written before it takes the next event, so a write that never
finishes holds every later event for that agent: a stall on a slow disk shows up as
`Session history overloaded: pending provider event count limit exceeded`, not as a
write error. The event being written holds two slots (its event and its row) and every
queued event holds one. Grep `daemon.log` for `Session history write is stalled` (after
10 s) and `Session event admission failed`; the second names the holder (pending run,
steer barrier, session queue, write tail).
