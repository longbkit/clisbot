...
(1,024 events / 8 MiB per agent) until they are written or released. The manager takes
the next event without waiting for the last one's row, so rows streamed during an fsync
share the next group commit (below). What it publishes waits instead, in the
[committed event outbox](../../../packages/server/src/server/agent/session-storage/committed-event-outbox.ts):
stream events, foreground turn waiters and plugin hooks receive each event in order, and
only after the rows written before it are committed. Once history fails, the queued
deliveries are dropped and no row is published. Agent state stays synchronous, because
in-process waiters read it against live manager state; a turn end is applied only after
the turn's rows are delivered, so no client, waiter or finished notice sees a turn end
ahead of its last row. A tool approval anchored to a row still being written records its
cursor only after that row commits.

Each held item holds one slot: a timeline event hands its slot to its row, and any other
event keeps its slot until what it published is delivered. A write that never finishes
therefore fills the budget with rows and held events for that agent, and shows up as
`Session history overloaded: pending provider event count limit exceeded`, not as a write
error. Grep `daemon.log` for `Session history write is stalled` (after 10 s) and
`Session event admission failed`; the second names the holder (pending run, steer
barrier, session queue, write tail).

Below the manager, the durable write path keeps one stalled session from taking the
daemon with it:

- **Admission** ([store-admission.ts](../../../packages/server/src/server/agent/session-storage/store-admission.ts)):
  the store refuses writes past 1,536 operations / 8 MiB per agent or 8,192 / 32 MiB
  overall, and counts reads apart, so a stalled agent refuses neither another agent's
  writes nor anyone's reads. The per-agent limits sit above the event budget, so a
  manager write that reserved an event is refused there first.
- **Group commit** ([group-commit.ts](../../../packages/server/src/server/agent/session-storage/group-commit.ts)):
  rows queued behind an in-flight append share the next append — one `writev`, one
  fsync, at most 256 rows / 1 MiB. A batch accepts rows only while it is the last queued
  operation for its agent, so nothing is reordered. Every row in it is acknowledged after
  that fsync or rejected with its error. The index learns about a batch only after its
  fsync; a failed append drops the cached index so the next read folds in what the log
  holds, as a restart would. File handles are opened per append and closed after it.
- **I/O limit** ([session-storage-io.ts](../../../packages/server/src/server/agent/session-storage/session-storage-io.ts)):
  every fs call holds a libuv threadpool thread, and a stuck fsync never returns it.
  With an explicit `UV_THREADPOOL_SIZE`, session-log appends and index checkpoints run at
  most half the threadpool at once, never fewer than 4
  (`PASEO_SESSION_LOG_WRITE_CONCURRENCY` overrides it). The supervisor entrypoint starts
  the daemon worker with `UV_THREADPOOL_SIZE=16` unless you set it, which gives 8 write
  slots; `npm run dev`, `paseo daemon start` (with or without `--foreground`), the desktop
  app, `npm start` and the Docker image all start through it. A process without an
  explicit size (an embedded `createPaseoDaemon`, tests) keeps writes unbounded: on the
  4-thread default a whole-append limit measured ~17% slower.
- **Index cost:** `events.index.json` keeps per-kind seq bounds (`seqRanges`), so reading
  a stream head no longer walks every pointer. The field is optional: an index without
  it, or one an older daemon advanced past it, recomputes it on load. A checkpoint
  rewrites the whole index, so it waits until the unsaved tail is at least a quarter of
  the pointers or as many bytes as the last checkpoint (never less than 64 rows /
  256 KiB); a crash rescans at most that tail.

The failures recover differently:

- **Failed write** (disk full, I/O error, or the store refusing a row with
  `Session storage overloaded: operation queue limit exceeded`): stays failed until the
  daemon restarts. Every row that reaches the store already has its seq, so even a
  refusal leaves a hole the agent must not write past. It replaces an event overload still
  waiting to recover.
- **Event overload** (the pending event budget refuses an event before it has a seq):
  nothing was written. The turn is interrupted with its error kept on the agent. When every held event is written
  or released, the agent reads and accepts prompts again (`Session event admission
recovered after overload`). While the write stays stuck, the agent stays locked.
...
| **AC7 — fast, safe writes**: batched sequential append per journal with I/O tasks and queue bounded by bytes and overload handling; record replacement is atomic and "persisted" only after fsync; disk-full / write error is reported clearly — no fake success, no silent drop.                                                                         | Partial                                 | Canonical fsync, immutable queued rows, injected ENOSPC and actual child SIGKILL recovery tests pass. Group commit, per-agent store admission and a process-wide write I/O limit are in place ([file-agent-timeline-store.test.ts](../../../packages/server/src/server/agent/session-storage/file-agent-timeline-store.test.ts)). 10 writers × 1,000 rows on macOS APFS (Sep 17, `--writers-only`, noisy shared disk): one outstanding row per writer 37.3–38.6 s → 36.9 s at the default threadpool (writes unbounded) and 31.5 s → 26.2 s at `UV_THREADPOOL_SIZE=16` (8 write slots), ~10.5k fsyncs either way; rows pipelined 100 at a time ~38 s / 10,661 fsyncs → 1.1 s / 281 fsyncs. Through `AgentManager` with the real store (Sep 17, 10 agents × 1,000 tool rows pushed 20 at a time, `PASEO_AGENT_MANAGER_WRITE_BENCHMARK` in [agent-manager-committed-delivery.test.ts](../../../packages/server/src/server/agent/agent-manager-committed-delivery.test.ts)): waiting for each event's row 36.2 s / 10,000 fsyncs → group commit 4.2–4.7 s / 1,000 fsyncs. Event-loop results and all transport/provider queues remain incomplete.                                                                 |
