...
| Channel, Member linked                      | Keep the channel id and scope; add the verified `memberId`.                                                         |
...
- Presentation: every actor has a "face" — `avatarUrl` when available (falling back on image
  error), otherwise a monogram (1–2 leading letters) from the display name with a deterministic
  colour seeded by `SessionActor.id`, never by the name, so the colour survives a rename. An
...
| AC2 — profile in a tab        | Hovering the name/avatar reveals the id; clicking opens a profile tab in the current workspace using the existing tab architecture. The first version is read-only: name, avatar, id, identity source/scope, and the verified Member when present. Clicking the same identity again reopens that tab; the same id under a different Hub/org/channel connection never opens the wrong person. Uses a snapshot the viewer is allowed to see when the profile is unlinked or cannot be loaded; never demands a new account. Closing, reopening, restoring and returning to chat preserve drafts and reading position. Touch and keyboard open the profile too — never hover-dependent. |
