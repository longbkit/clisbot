...
- Upstream Hub now has organization-owned, revisioned Triggers; explicit `from_users: ["*"]`;
  GitHub, Slack, Discord, Linear, and manual sources; and multiple connection records beneath each
  provider application. The remaining product gap is conversational rather than basic dispatch:
  provider threads do not become durable Paseo Agent sessions, external participants cannot use the
  same granular Project/tool-approval policy as Paseo Members, and the upstream provider-trigger
  path is not the multi-account, two-way Channel control plane required by Clisbot. See the
  [current Hub UI proposal](../audits/2026-09-01-unified-client-hub-configuration-ui.md) and the
  [dated baseline audit](../audits/2026-08-23-paseo-hub.md).
...
The dated gap analysis [audits/2026-08-23-paseoclaw-fusion-gaps.md](../audits/2026-08-23-paseoclaw-fusion-gaps.md) turns this posture into sequenced proposals — workspace init templates, a same-machine channel path, and the Hub trigger-model fixes that follow them — each with its code seam, isolation flags, and verification criteria. The channel proposal it spawned is owned by the [channel-reuse plan](../audits/2026-08-23-openclaw-channel-reuse-plan.md): the channel control plane lives in the Hub and its initial embedded/team forms drive the daemon through existing trusted-client RPCs. Separately, upstream Hub Trigger execution uses its enrolled service principal and the semantic `hub.execute` permission; managed interactive access adds a resource boundary without replacing either transport. The build/publish/onboarding details are in the [hub-integration implementation doc](../audits/2026-08-24-hub-integration-implementation.md). The P0 verticals' supply model changed with the 2026-08-26 pull — Slack and Telegram are now first-party in-repo packages (`packages/channels/*`), the OpenClaw pinned dists demoted to upstream sync references: [2026-08-26-in-repo-channel-verticals.md](../audits/2026-08-26-in-repo-channel-verticals.md).
