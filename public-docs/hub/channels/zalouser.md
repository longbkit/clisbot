...
Then link it from the app: **Channels → Connections**, open the Connection with **Manage**, and use its QR operations. The flow is start → show the QR → poll → linked. Confirm the reported user id is the account you meant before you point a Route at it.
