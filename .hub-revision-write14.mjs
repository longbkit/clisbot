#!/usr/bin/env node
// Wave-6d live-E2E channel revision writer (fork of `.hub-revision-write13.mjs`,
...
//   telegram-tool -> attach the `message` tool to the Telegram topic route
//                    (`outbound.path: tool`). Wave 6c found no
//                    `/mcp/channel/<token>` traffic in the Telegram windows:
//                    the route posted the final answer through the reply path
//                    only, so the model had no channel tool to carry a
//                    presentation (scenario 6.6). Nothing else moves —
//                    `sync.streaming`, `interaction`, `audience`, the agent
//                    (`claude`/`claude-sonnet-5`) and every other file are
//                    carried through untouched.
//
// Usage: node .hub-revision-write14.mjs dump|telegram-tool
...
if (!["dump", "telegram-tool"].includes(scenario ?? "")) {
  fail("usage: node .hub-revision-write14.mjs dump|telegram-tool");
}

const TELEGRAM_FILE = ".paseo/channels/telegram/onboarding-telegram.yml";
const ROUTE_KIND = "topic";
...
function transform(files) {
  ...
  for (const route of Array.isArray(doc?.routes) ? doc.routes : []) {
    if (route?.match?.kind !== ROUTE_KIND) continue;
    const before = route.outbound?.path ?? "none";
    route.outbound = { ...(route.outbound ?? {}), path: "tool" };
    hits += 1;
    log(`${TELEGRAM_FILE}: routes[${ROUTE_KIND}].outbound.path ${before} -> tool`);
  }
  if (hits === 0) fail(`no ${ROUTE_KIND} route found — inspect the dump first`);
