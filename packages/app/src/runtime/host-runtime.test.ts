...
describe("HostRuntimeStore", () => {
  ...
  it("adopts an existing Host when the Hub proves the same daemon identity", async () => {
    ...
    // Adoption is proved by the daemon key, so the offer has to carry the same one as the
    // Host it should merge into — and the same relay, or it would add a second connection.
    const profile = await store.upsertManagedConnectionFromOffer({
      offer: makeOffer({
        daemonPublicKeyB64: "manual-public-key",
        relay: { endpoint: "manual-relay.example.test:443", useTls: true },
      }),
      management,
    });
