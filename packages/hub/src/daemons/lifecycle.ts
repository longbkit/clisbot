...
export class DaemonDispatchLifecycle {
  ...
  private notifyPrelaunchFailure(
    ...
    const tracked = after
      ...
      .catch((error: unknown) => {
        this.report(error, "daemon.prelaunch.notification", {
  ...
  private async prepareDispatch(
    ...
    if (
      ...
      run.workflowId !== intent.workflowId ||
      config?.organizationId !== intent.organizationId ||
  ...
  private configurationRevisionForIntent(intent: LaunchMachineIntent) {
    return this.options.database.findOrganizationTriggerRevision(
      intent.workflowId,
  ...
  private startDurableDispatch(
    ...
    const tracked = Promise.resolve(accepted)
      ...
      .catch((error: unknown) => {
        this.report(error, "daemon.dispatch.durable", {
  ...
  private async handleDaemonEvent(
    ...
    if (event.type === "agent_stream") {
      ...
      if (
        ...
        allowsChannelReply(channelExecution)
      ) {
        await this.options.onWorkflowChannelStream?.({
  ...
  async completeAgentExecutionFromCallback(
    ...
    // The MCP HTTP response can win the race against the daemon's final
    // assistant/timeline frames. When dispatch cleanup is deferred, keep the
    // execution subscription alive for every Hub action (including `null`),
    // so channel relay can drain the final answer before terminal + idle
    // acknowledgements release it.
  ...
  async recoverAgentExecutionDeadlines(): Promise<void> {
    ...
    for (const execution of executions) {
      try {
        ...
        this.report(error, "daemon.execution.deadline.recover", {
          executionId: execution.id,
        });
      }
    }
  ...
  private async notifyExecutionTerminal(execution: AgentExecutionRecord): Promise<void> {
    ...
    if (provider !== undefined) {
      await notifyAgentExecutionTerminal({
        ...
        this.report(error, "daemon.provider.terminal-cleanup", {
          executionId: execution.id,
        });
      });
    }
  ...
  private async acquireAndSpawnAgentWithoutTimeout(
    ...
    const trackHandler = (operation: Promise<void>): Promise<void> => {
      const tracked = operation.catch((error: unknown) => {
        if (!disposed) {
          this.report(error, "daemon.launch.event", {
            executionId: input.executionId,
          });
        }
      });
    ...
    try {
      ...
      void (async () => {
        ...
        this.report(error, "daemon.dispatch.watch", {
          executionId: input.executionId,
        });
      });
      return agent.id;
...
function toDaemonDispatchFailure(error: unknown): DaemonDispatchFailure {
  ...
  if (error instanceof DaemonCreateRejectedError) {
    return new DaemonDispatchFailure(daemonCreateFailureReason(error), {
      cause: error,
    });
  }
...
function channelReplyContext(context: unknown): { channel: object; outbound: object } | undefined {
  if (
    ...
    context === null ||
    Reflect.get(context, "provider") !== "channel"
  ) {
    return undefined;
  }
...
function allowsChannelReply(execution: AgentExecutionRecord): boolean {
  ...
  if (Reflect.get(context, "provider") !== "channel") return false;
  const channel = Reflect.get(context, "channel");
