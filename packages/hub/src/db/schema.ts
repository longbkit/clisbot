...
import {
  INVITATION_ROLES,
  ORGANIZATION_ROLES,
} from "../auth/organization-contract.js";
...
export const agentExecutionStatus = pgEnum(
...
export const attachmentCapabilities = pgTable(
  ...
  (table) => [
    ...
    index("attachment_capabilities_receipt_idx").on(
      table.providerEventReceiptId,
    ),
    check(
    ...
    foreignKey({
      ...
      foreignColumns: [
        ...
        providerEventReceipts.organizationId,
      ],
...
export const projects = pgTable(
  "projects",
  {
    ...
    slug: text().notNull(),
    status: text()
      .$type<(typeof PROJECT_STATUSES)[number]>()
      .default("active")
      .notNull(),
    ...
    activeConfigurationRevisionId: uuid(
      "active_configuration_revision_id",
    ).references((): AnyPgColumn => projectConfigurationRevisions.id),
  },
  (table) => [
...
export const projectConfigurationRevisions = pgTable(
  "project_configuration_revisions",
  {
    ...
    sourceKind: text("source_kind")
      .$type<(typeof CONFIGURATION_SOURCE_KINDS)[number]>()
      .notNull(),
...
export const organizationTriggers = pgTable(
  ...
  (table) => [
    ...
    uniqueIndex("organization_triggers_id_organization_unique").on(
      table.id,
      table.organizationId,
    ),
...
export const organizationTriggerRevisions = pgTable(
  ...
  (table) => [
    ...
    ).on(table.id, table.triggerId, table.organizationId),
    ...
    foreignKey({
      ...
      foreignColumns: [
        organizationTriggers.id,
        organizationTriggers.organizationId,
      ],
...
export const triggerRuns = pgTable(
  "trigger_runs",
  {
    ...
    outcome: text()
      ...
      .default("accepted"),
    status: text()
      .$type<"running" | "succeeded" | "failed" | "timed_out" | "rejected">()
      .notNull(),
    prompt: text().notNull(),
    ...
    deadlineKind: text("deadline_kind").$type<
      "step_hard" | "step_idle" | "whole_run"
    >(),
    failureReason: text("failure_reason"),
    reactionState: jsonb("reaction_state"),
    terminalNotificationPendingAt: timestamp(
      "terminal_notification_pending_at",
      {
        withTimezone: true,
      },
    ),
    terminalNotificationDeliveredAt: timestamp(
      "terminal_notification_delivered_at",
      {
        withTimezone: true,
      },
    ),
    terminalNotificationLeaseExpiresAt: timestamp(
  ...
  (table) => [
    ...
    index("trigger_runs_status_deadline_idx").on(
      table.status,
      table.deadlineAt,
    ),
    index("trigger_runs_workflow_created_idx").on(
      table.workflowId,
      table.createdAt.desc(),
    ),
...
export const workflowStepRuns = pgTable(
  ...
  (table) => [
    uniqueIndex("workflow_step_runs_trigger_ordinal_unique").on(
      table.triggerRunId,
      table.ordinal,
    ),
    uniqueIndex("workflow_step_runs_trigger_step_unique").on(
      table.triggerRunId,
      table.stepId,
    ),
    ...
    index("workflow_step_runs_trigger_status_idx").on(
      table.triggerRunId,
      table.status,
    ),
    check(
...
export const workflowWakeups = pgTable(
  ...
  (table) => [
    index("workflow_wakeups_available_lease_idx").on(
      table.availableAt,
...
export const daemons = pgTable(
  "daemons",
  {
    ...
    registeredByCliCredentialId: uuid(
      "registered_by_cli_credential_id",
    ).references((): AnyPgColumn => organizationCliCredentials.id, {
      onDelete: "set null",
    }),
    status: text().$type<"active" | "revoked">().notNull(),
    presence: text()
      .$type<"offline" | "connected">()
      .default("offline")
      .notNull(),
  ...
  (table) => [
    ...
    uniqueIndex("daemons_id_organization_unique").on(
      table.id,
      table.organizationId,
    ),
    uniqueIndex("daemons_organization_slug_unique").on(
      table.organizationId,
      table.slug,
    ),
    foreignKey({
    ...
    check(
      ...
      sql`${table.presence} in ('offline', 'connected')`,
    ),
  ],
);
...
export const cliAuthorizations = pgTable(
  "cli_authorizations",
  {
    ...
    status: text()
      .$type<"pending" | "approved" | "denied" | "expired" | "disclosed">()
      .notNull(),
  ...
  (table) => [
    index("cli_authorizations_fingerprint_idx").on(
      table.fingerprintVerifier,
      table.expiresAt,
    ),
    index("cli_authorizations_status_expiry_idx").on(
      table.status,
      table.expiresAt,
    ),
    check(
...
export const agentExecutions = pgTable(
  "agent_executions",
  {
    ...
    outputDeliveryAttempts: jsonb("output_delivery_attempts")
      .notNull()
      .default({}),
  ...
  (table) => [
    ...
    index("agent_executions_workflow_started_at_idx").on(
      table.workflowId,
...
export const workflowAgentReuseBindings = pgTable(
  ...
  (table) => [
    ...
    index("workflow_agent_reuse_bindings_execution_idx").on(
...
export const organizationConnectionAttempts = pgTable(
  "organization_connection_attempts",
  {
    id: uuid().defaultRandom().primaryKey(),
    provider: text()
      .$type<"github" | "discord" | "slack" | "linear">()
      .notNull(),
    phase: text()
      .$type<
        | "github_setup"
    ...
    configurationEnvelope: jsonb("configuration_envelope")
      .$type<CredentialEnvelope>()
      .notNull(),
    expectedConfigurationVersion: integer("expected_configuration_version"),
    activateConfiguration: boolean("activate_configuration")
      .default(false)
      .notNull(),
    createdAt: timestamp("created_at", { withTimezone: true })
      .defaultNow()
...
export const githubConnections = pgTable(
  ...
  (table) => [
    uniqueIndex("github_connections_installation_unique").on(
      table.installationId,
    ),
    uniqueIndex("github_connections_organization_slug_unique").on(
      table.organizationId,
      table.slug,
    ),
    uniqueIndex("github_connections_id_organization_unique").on(
      table.id,
      table.organizationId,
    ),
...
export const discordConnections = pgTable(
  ...
  (table) => [
    uniqueIndex("discord_connections_id_organization_unique").on(
      table.id,
      table.organizationId,
    ),
...
export const slackConnections = pgTable(
  ...
  (table) => [
    uniqueIndex("slack_connections_id_organization_unique").on(
      table.id,
      table.organizationId,
    ),
    ...
    uniqueIndex("slack_connections_organization_slug_unique").on(
      table.organizationId,
      table.slug,
    ),
  ],
...
export const linearConnections = pgTable(
  "linear_connections",
  {
    ...
    credentialEnvelope: jsonb("credential_envelope")
      .$type<CredentialEnvelope>()
      .notNull(),
    refreshTokenAvailable: boolean("refresh_token_available")
      .default(false)
      .notNull(),
    accessTokenExpiresAt: timestamp("access_token_expires_at", {
      withTimezone: true,
  ...
  (table) => [
    uniqueIndex("linear_connections_id_organization_unique").on(
      table.id,
      table.organizationId,
    ),
    uniqueIndex("linear_connections_organization_slug_unique").on(
      table.organizationId,
      table.slug,
    ),
...
export const projectConfigurationSources = pgTable(
  "project_configuration_sources",
  {
    ...
    automaticDeploymentEnabled: boolean("automatic_deployment_enabled")
      .default(false)
      .notNull(),
...
export const auditEvents = pgTable(
  ...
  (table) => [
    ...
    check(
      ...
      sql`${table.actorKind} in ('user', 'github', 'system')`,
    ),
    foreignKey({
      columns: [table.projectId, table.organizationId],
      foreignColumns: [projects.id, projects.organizationId],
...
export const runtimeConfiguration = pgTable(
  "runtime_configuration",
  {
    ...
    authSecretEnvelope: jsonb("auth_secret_envelope")
      .$type<CredentialEnvelope>()
      .notNull(),
    createdAt: timestamp("created_at", { withTimezone: true })
      .defaultNow()
  ...
  (table) => [
    check("runtime_configuration_singleton_check", sql`${table.singleton}`),
  ],
);
...
export const organizationApiKeys = pgTable(
  "organization_api_keys",
  {
    ...
    scopes: text()
      .array()
      .$type<readonly (typeof API_KEY_SCOPES)[number][]>()
      .notNull(),
    createdByUserId: text("created_by_user_id").references(() => users.id, {
      onDelete: "set null",
    }),
...
export const ENTITLEMENT_CHANGE_SOURCES = [
  "provisioning",
  "plan_stamp",
  "override",
] as const;
...
export const entitlementChanges = pgTable(
  "entitlement_changes",
  {
    ...
    actor: text(),
    source: text()
      .$type<(typeof ENTITLEMENT_CHANGE_SOURCES)[number]>()
      .notNull(),
    before: jsonb(),
    after: jsonb().notNull(),
    reason: text(),
...
export const organizationUsage = pgTable(
  ...
  (table) => [
    ...
    index("organization_usage_organization_meter_idx").on(
      table.organizationId,
      table.meter,
    ),
...
export const billingPlanPrices = pgTable(
  "billing_plan_prices",
  {
    ...
    interval: text()
      .$type<(typeof BILLING_PLAN_PRICE_INTERVALS)[number]>()
      .notNull(),
  ...
  (table) => [
    ...
    check(
      ...
      sql`${table.interval} in ('monthly', 'annual')`,
    ),
  ],
);
...
export const THREAD_BINDING_STATUSES = [
  "pending",
  "bound",
  "abandoned",
] as const;
export const DELIVERY_LEDGER_STATUSES = [
  "recorded",
  "posted",
  "failed",
  "consumed",
...
export const channelConfigurationRevisions = pgTable(
  ...
  (table) => [
    uniqueIndex(
      "channel_configuration_revisions_organization_version_unique",
    ).on(table.organizationId, table.version),
...
export const threadBindings = pgTable(
  ...
  (table) => [
    ...
    index("thread_bindings_account_created_idx").on(
      table.accountId,
      table.createdAt.desc(),
    ),
    check(
    ...
    check(
      "thread_bindings_channel_check",
      sql`${table.channel} in ('slack', 'telegram')`,
    ),
    check(
...
export const deliveryLedger = pgTable(
  ...
  (table) => [
    ...
    index("delivery_ledger_account_created_idx").on(
      table.accountId,
    ...
    check(
      "delivery_ledger_direction_check",
      sql`${table.direction} in ('in', 'out')`,
    ),
    check(
      "delivery_ledger_channel_check",
      sql`${table.channel} in ('slack', 'telegram')`,
    ),
