import { ChannelCommandReceiptStore } from "./channel-command-receipts.js";
...
export class ChannelCommandStore extends ChannelCommandReceiptStore {
  async listCommands(key: ChannelAccountKey): Promise<ChannelCommandRecord[]> {
    return this.database
      .select(commandColumns)
      .from(schema.channelCommands)
      .where(and(...commandAccountWhere(key)))
      .orderBy(asc(schema.channelCommands.name));
  }

  async findCommand(
    key: ChannelAccountKey,
    name: string,
  ): Promise<ChannelCommandRecord | undefined> {
  ...
  async setCommand(
    ...
    const [row] = await this.database
      .insert(schema.channelCommands)
      .values({ ...key, ...input, updatedAt })
      .onConflictDoUpdate({
        target: [
          schema.channelCommands.organizationId,
          schema.channelCommands.channel,
          schema.channelCommands.accountId,
          schema.channelCommands.name,
        ],
...
function commandAccountWhere(key: ChannelAccountKey) {
  return [
    eq(schema.channelCommands.organizationId, key.organizationId),
    eq(schema.channelCommands.channel, key.channel),
    eq(schema.channelCommands.accountId, key.accountId),
  ];
}
