...
CREATE TABLE "delivery_ledger" (
	...
	"failure_reason" text,
	CONSTRAINT "delivery_ledger_status_check" CHECK ("delivery_ledger"."status" in ('recorded', 'posted', 'failed', 'consumed')),
	CONSTRAINT "delivery_ledger_direction_check" CHECK ("delivery_ledger"."direction" in ('in', 'out')),
	...
	CONSTRAINT "delivery_ledger_attempts_check" CHECK ("delivery_ledger"."attempts" >= 1),
	CONSTRAINT "delivery_ledger_inbound_shape_check" CHECK (("delivery_ledger"."direction" = 'in'
        and "delivery_ledger"."event_turn_id" = ''
        and "delivery_ledger"."sequence" = 0
        and "delivery_ledger"."external_message_id" is not null
        and "delivery_ledger"."status" in ('recorded', 'consumed')
        and "delivery_ledger"."consumed_at" is not null = ("delivery_ledger"."status" = 'consumed')
        and "delivery_ledger"."turn_id" is not null = ("delivery_ledger"."status" = 'consumed'))
        or ("delivery_ledger"."direction" = 'out'
        and "delivery_ledger"."status" in ('recorded', 'posted', 'failed')))
);
--> statement-breakpoint
...
CREATE UNIQUE INDEX "thread_bindings_account_thread_unique" ON "thread_bindings" USING btree ("organization_id","account_id","external_conversation_id","external_thread_id");--> statement-breakpoint
