...
describe("discord group policy", () => {
  // Skipped: the sender-specific half asserts upstream's `toolsBySender`
  // resolution, which core's carried `resolveScopeToolsPolicy` narrows away
  // (D-CORE-222 — the OpenClaw sender-policy resolver is not ported). The
  // channel-over-guild precedence the same case covers is asserted by the
  // account-scoped case below.
  it.skip("prefers channel policy, then guild policy, with sender-specific overrides", () => {
    const discordCfg = {
      channels: {
        discord: {
          token: "discord-test",
