...
export async function handleDiscordMessageSendAction(ctx: DiscordMessagingActionContext) {
  switch (ctx.action) {
    ...
    case "sendMessage": {
      ...
      if (asVoice) {
        // D-DC-004: the native voice-message send is omitted with
        // `send.voice.ts` / `voice-message.ts` (ffmpeg + opus transcoding). The
        // action fails loudly rather than silently degrading to a file upload.
        throw new Error(
          "Discord voice messages are not supported by this build (send.voice is not ported).",
        );
      }
