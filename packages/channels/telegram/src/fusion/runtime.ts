// Fusion-owned runtime adapter for `extensions/telegram/src/runtime.ts` (D-TG-014).
//
...
/** Builds the `TelegramRuntime` the ported stores read, backed by the Hub. */
export function createTelegramRuntimeFromHost(host: HostRuntime): TelegramRuntime {
...
/**
 * The channel's subsystem log sink: `[telegram/…]` lines go to the logger of
...
function installSubsystemLogSink(): void {
  installChannelSubsystemLogSink({
    ...
    currentAccountId: currentTelegramAccountId,
  });
}
 ...
 */
export function installTelegramRuntime(host: HostRuntime, accountId = ""): void {
  accountLoggers.set(
    accountId,
  ...
  setTelegramRuntime(createTelegramRuntimeFromHost(host), accountId);
}
...
export function disposeTelegramRuntime(accountId = ""): void {
  ...
  if (accountLoggers.size === 0) clearChannelSubsystemLogSink("telegram");
}
