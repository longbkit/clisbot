// Replays the recorded differential corpus against the local Telegram pure
// functions. A failure here means a ported function changed behaviour since
// the corpus was recorded — regenerate with
// `node --import tsx scripts/channel-differential-fixtures.mjs generate --pkg telegram`
...
import { telegramDifferentialCases } from "./__fixtures__/upstream-differential.cases.js";

const corpus = readDifferentialCorpus(
  new URL("./__fixtures__/upstream-differential.json", import.meta.url),
);

describe("Telegram upstream differential corpus", () => {
  it("was recorded against the ported upstream baseline", () => {
    expect(corpus.channel).toBe("telegram");
  ...
  it("names an upstream test file for every case", () => {
    const orphans = corpus.cases.filter(
      (fixture) => !fixture.upstreamTestFile.startsWith("extensions/telegram/"),
    );
    expect(orphans).toEqual([]);
  });

  it("reproduces every recorded output", () => {
    expect(replayDifferentialCorpus(corpus, telegramDifferentialCases)).toEqual([]);
  });
});
