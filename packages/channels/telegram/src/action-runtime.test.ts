...
function installTopicNameStoreForTest() {
  ...
  setTelegramRuntime({
    ...
    channel: {},
    // Fusion's `TelegramRuntime` also declares `openSyncKeyedStore`; this mock
    // installs only the async store the topic-name cache reads (D-TG-014).
...
describe("handleTelegramAction", () => {
  ...
  // D-TG-032: dropped case "persists sendMessage action deliveries before
  // Telegram platform send". It drives OpenClaw's own durable delivery queue
  // through `openclaw/plugin-sdk/plugin-test-runtime` (test plugin registry,
  // `readQueuedDeliveryEntriesForTest` over a state directory) and asserts the
  // queued row shape. The Hub owns durable delivery in Fusion (goal slices 1-3,
  // 8) and that queue is `channel_ingress_queue` + the Hub outbound ledger, so
  // the invariant is covered by `packages/hub/src/channels/*` instead.
