// upstream: extensions/zalo/src/accounts.test.ts@5d8067a4483
...
// Adapted: `channel.ts` is omitted (the Hub owns the plugin definition), and
// what it wired as `config.isConfigured` IS `isZaloAccountConfigured`.
...
describe("Zalo account SecretRef inspection", () => {
  ...
  it.each([
    ...
    expect(isZaloAccountConfigured(account)).toBe(entry.configured);
  ...
  // Adapted (D-ZL-019): upstream's strict mode THROWS on an unresolved
  // SecretRef because the OpenClaw host resolves refs lazily at read time. In
  // Fusion the Hub resolves every credential before it hands the carrier over,
  // so an unresolved ref can only come from authored config: the core boundary
  // reports it as `configured_unavailable` and the account starts blocked with
  // a diagnostic instead of throwing inside a send.
  it("keeps direct account resolution strict", () => {
    expect(
      resolveZaloAccount({ cfg: { channels: { zalo: { botToken: unresolvedRef } } } }),
    ).toMatchObject({ token: "", tokenStatus: "configured_unavailable" });
  });
