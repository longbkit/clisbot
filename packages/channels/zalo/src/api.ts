// upstream: extensions/zalo/src/api.ts@5d8067a4483
/**
 * Zalo Bot API client
...
export class ZaloApiError extends Error {
  ...
  get isRetryable(): boolean {
    ...
    return this.isPollingTimeout || this.isRateLimited || (this.errorCode ?? 0) >= 500;
  }
}
...
export async function callZaloApi<T = unknown>(
  ...
  try {
    ...
    try {
      ...
      data = await readProviderJsonResponse<ZaloApiResponse<T>>(response, `zalo.${method}`);
    } catch (error) {
      if (!response.ok) {
        // Keep the SDK's bounded error detail while preserving polling's status classification.
        throw new ZaloApiError(
          error instanceof Error
            ? error.message
            : `Zalo API error: ${method} (HTTP ${response.status})`,
          response.status,
        );
      }
      throw error;
    ...
    if (!data.ok) {
      throw new ZaloApiError(
        data.description ?? `Zalo API error: ${method} (HTTP ${response.status})`,
