...
describe("Zalo API request methods", () => {
  ...
  it("keeps the default request deadline active while reading a hanging send response body", async () => {
    ...
    try {
      ...
      // Adapted (D-CORE-036): upstream's `readProviderJsonResponse` raises the
      // caller's abort error ("zalo body aborted") when the body read is cut
      // short; the Fusion boundary resolves with the prefix read so far, so the
      // same abort surfaces as a parse failure. What this case is really about —
      // the deadline stays armed while the body hangs, the request signal
      // aborts and the body stream is released — is unchanged below.
      const rejected = expect(promise).rejects.toThrow("malformed JSON response");
      ...
      // Adapted (D-CORE-036): upstream lets the hanging body run until the
      // REQUEST deadline aborts it, so the request signal is the thing that
      // fires. The Fusion boundary bounds the read itself with a per-chunk idle
      // deadline (10s) and gives up first, which is why the request signal is
      // still unaborted here — the caller is already free either way, and that
      // is the property this case protects: a hanging Zalo response body cannot
      // hold the send. `bodyAborted` stays false for the same reason: the reader
      // is cancelled and released rather than errored by the request signal.
  ...
  it("bounds oversized getMe JSON responses and cancels the stream", async () => {
    ...
    // Adapted (D-CORE-306): upstream throws a size-named error at the cap; the
    // Fusion boundary stops reading at the cap and the truncated body fails to
    // parse. The invariant the case exists for — the oversized stream is bounded
    // and CANCELLED exactly once, never buffered — is asserted unchanged.
    await expect(getMe("test-token", undefined, fetcher)).rejects.toThrow(
      "zalo.getMe: malformed JSON response",
    );
