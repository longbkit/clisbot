...
// lifecycle hands its updates to. One store per channel process; the Hub drives
// one account per vertical instance today, but the map costs nothing and keeps
// multi-account starts from crossing wires. Mirrors the Google Chat, Discord and
...
export function getHostRuntime(): HostRuntime {
  if (hostRuntime === undefined) {
    throw new Error("zalo channel runtime not set (call entry.setChannelRuntime first)");
  }
  return hostRuntime;
}
...
export function registerAccountInbound(
  ...
  if (entry === undefined || entry.hostRuntime !== runtime) {
    const processor = createInboundEventProcessor({
      ...
      logger: runtime.logging.getChildLogger({ channel: "zalo", accountId }),
    });
    entry = {
      hostRuntime: runtime,
      processor,
...
export function getAccountRuntime(accountId: string): AccountInbound {
  ...
  if (entry === undefined) {
    throw new Error(`zalo account "${accountId}" has no inbound processor (not started?)`);
  }
  return entry;
}
