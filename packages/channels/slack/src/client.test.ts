// upstream: extensions/slack/src/client.test.ts@5d8067a4483
...
describe("slack proxy dispatcher", () => {
  ...
  // D-030: the debug-proxy capture patch and the undici EnvHttpProxyAgent
  // precedence live in OpenClaw's net stack; the Fusion fetch boundary reads the
  // proxy env with upstream's precedence but installs no capture patch.
  it.skip("keeps the capture-patched global fetch with ambient proxy env", async () => {
    process.env.HTTPS_PROXY = "http://proxy.example.com:3128";
  ...
  // precedence live in OpenClaw's net stack; the Fusion fetch boundary reads the
  // proxy env with upstream's precedence but installs no capture patch.
  it.skip("treats empty lowercase https_proxy as authoritative over uppercase", () => {
