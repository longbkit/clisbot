// upstream: extensions/slack/src/errors.test.ts@5d8067a4483
...
describe("formatSlackError", () => {
  ...
  it("redacts token-shaped values before returning", () => {
    ...
    // D-CORE-002: upstream's redactor masks a token in place as
    // `xoxb-1…cdef` (prefix/suffix preserved for operator correlation).
    // Fusion's carried `redactSensitiveText` replaces the whole match with
    // `[redacted]`. The security property the case pins — the raw token never
    // reaches the formatted message — is asserted above and still holds.
