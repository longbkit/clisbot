...
/**
 * Tell Bolt's socket client that this stop is deliberate.
 *
 * The patched reconnect scheduler above checks `shuttingDown` when its timer
 * fires, so anything that begins a stop must set the flag BEFORE it waits on
 * the disconnect — not after (D-042).
 */
export function markSlackSocketShuttingDown(app: unknown): void {
