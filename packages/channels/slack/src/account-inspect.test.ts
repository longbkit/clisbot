...
describe("inspectSlackAccount", () => {
  ...
  it("keeps an active socket app-token ref unavailable and operational resolution strict", () => {
    ...
    expect(account).not.toHaveProperty("signingSecretStatus");
    // D-CORE-257: upstream's `resolveSlackAccount` throws here because
    // `normalizeResolvedSecretInputString` recognizes a configured-but-
    // unresolved SecretRef and refuses it strictly. Fusion's Hub resolves
    // credentials before the drive-time `cfg` reaches the vertical, so a
    // remaining ref reads as absent instead of as a strict failure. The
    // `configured_unavailable` status above still reports it.
  });
