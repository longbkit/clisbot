// upstream: extensions/zalouser/src/zalo-js.credentials.test.ts@5d8067a4483
// D-ZU-004: the harness moves with the boundary. Upstream drives OpenClaw's
// plugin-state test runtime through `setZalouserRuntime` and scopes it with a
// temp `OPENCLAW_STATE_DIR`; Fusion installs an in-memory
// `ZalouserSessionStore`. Every assertion — refresh-after-logout, the
// revocation marker, the credential signature, the persistence modes — is
// unchanged, because the ported credential closure is unchanged.
...
/** Upstream's `withEnvAsync({ OPENCLAW_STATE_DIR }, run)` scoped the OpenClaw
 * state dir the plugin-state store opened under. Fusion's store is injected, so
 * the scope is just the run — the env value is kept in the call sites so the
 * ported cases stay line-for-line comparable. */
async function withEnvAsync<T>(_env: NodeJS.ProcessEnv, run: () => Promise<T>): Promise<T> {
  return await run();
}
import {
...
describe("zalouser credential persistence", () => {
  ...
  // D-ZU-005: upstream's last case asserts the credential write lands in the
  // OpenClaw plugin-state SQLite file and does NOT recreate the retired
  // per-profile credentials blob on disk. Fusion has neither file: the record
  // goes to the injected `ZalouserSessionStore`, and the legacy path helpers are
  // dropped with it. The invariant the case exists for — a write reaches the
  // store, and nothing is written to a credentials file — is asserted against
  // the store instead.
  it("writes the credential record to the injected session store and no file", async () => {
    const stateDir = await mkdtemp(path.join(os.tmpdir(), "openclaw-zalouser-credentials-"));
    const profile = "store-only";
    ...
    try {
      ...
      expect(store.rows.get(zalouserCredentialStoreKey(profile))).toMatchObject({
        profile,
        imei: "api-imei",
      });
      await expect(access(path.join(stateDir, "credentials"))).rejects.toMatchObject({
        code: "ENOENT",
      });
    } finally {
      await removeCredentialStateDir(stateDir);
