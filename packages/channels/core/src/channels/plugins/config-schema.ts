// Fusion-owned partial port of `src/channels/plugins/config-schema.ts` (D-CORE-339).
//
// Upstream's module also builds a channel config schema straight from JSON
// Schema (`buildJsonChannelConfigSchema` / `safeParseJsonSchema`), which runs
// OpenClaw's plugin manifest validator (TypeBox compile cache, plugin origin
// cache, terminal-safe text). No in-repo vertical declares its config that way —
// they all use the zod builders below — so that half is omitted; everything else
// is upstream's, line for line.
/**
