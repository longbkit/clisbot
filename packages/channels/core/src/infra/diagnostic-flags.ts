// upstream: src/infra/diagnostic-flags.ts@5d8067a4483
...
import { normalizeUniqueStringEntriesLower } from "../normalization-core/string-normalization.js";
...
export function resolveDiagnosticFlags(
  ...
  // D-CORE-205: the host config is opaque in Fusion, so the `diagnostics.flags`
  // read is narrowed here instead of being typed by `OpenClawConfig`.
  const diagnostics = cfg?.diagnostics as { flags?: unknown } | undefined;
  const configFlags = Array.isArray(diagnostics?.flags) ? (diagnostics.flags as string[]) : [];
