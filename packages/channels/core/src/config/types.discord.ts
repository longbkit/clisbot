// upstream: src/config/types.discord.ts@5d8067a4483
// D-CORE-301: upstream composes `DiscordAccountConfig` out of
// `CommonChannelMessagingConfig`, `ChannelBotInteractionConfig`,
// `ChannelReactionConfig`, `ChannelExecApprovalConfig`, `ProviderCommandsConfig`,
// `SecretInput`, `TtsConfig` and the group tool-policy types — i.e. the whole
// OpenClaw `config.json` type graph. Fusion's Hub owns channel configuration, so
// the composed bases are re-declared here with the fields the ported Discord code
// reads plus an open index signature, exactly as `types.slack.ts` and
// `types.telegram.ts` already do. Everything from `DiscordStreamMode` down is
...
/** Upstream `src/config/types.tools.ts`; narrowed to the allow/deny lists the ported code reads. */
...
/** Upstream `src/config/types.messages.ts`. */
export type ProviderCommandsConfig = boolean | "auto" | { [key: string]: unknown };

/** Upstream `src/config/types.channel-messaging-common.ts`. */
export type ChannelExecApprovalTarget = "dm" | "channel" | "both";
export type ChannelExecApprovalConfig<_TTarget = ChannelExecApprovalTarget> = {
  enabled?: boolean;
...
/** Upstream `src/config/types.tts.ts`; opaque here — Discord voice is omitted. */
export type TtsConfig = { [key: string]: unknown };

/** Upstream `src/config/types.discord-presence.ts`; opaque here. */
export type DiscordPresenceEventsConfig = { [key: string]: unknown };

/** Upstream `src/config/types.base.ts` preview-streaming shape. */
export type ChannelPreviewStreamingConfig = ChannelStreamingConfig;

/** Upstream `src/config/types.channel-messaging-common.ts` bot-interaction block. */
export type ChannelBotLoopProtectionConfig = {
  /** Max consecutive bot-authored turns before the loop guard trips. */
  maxConsecutiveBotMessages?: number;
  /** Cooldown after the guard trips. */
  cooldownMs?: number;
  [key: string]: unknown;
};
...
/** Upstream `src/config/types.channel-messaging-common.ts` reaction block. */
...
/** Upstream `src/config/types.channel-messaging-common.ts` messaging base. */
export type CommonChannelMessagingConfig<
  TCapabilities = string[],
  TAllowFromEntry = string | number,
  TDefaultTo = string,
  TStreaming = ChannelStreamingConfig,
> = {
