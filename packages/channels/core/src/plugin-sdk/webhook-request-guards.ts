// upstream: src/plugin-sdk/webhook-request-guards.ts@5d8067a4483
// D-CORE-320: two OpenClaw host couplings are cut at the smallest boundary.
// `runDetachedWebhookWork` upstream reserves a root in OpenClaw's process-wide
// gateway work-admission store so a restart drain waits for detached webhook
// work; the Hub owns drain lifecycle in Fusion (`hub/src/channels/ingress/drain.ts`),
// so the reservation is dropped and the deferral shape kept. The
// `resolveAcceptedBrowserOrigin` re-export (OpenClaw gateway origin check) is
// omitted: no ported caller reads it.
// Webhook request guards validate incoming HTTP requests before plugin webhook dispatch.
import type { IncomingMessage, ServerResponse } from "node:http";
import { resolveIntegerOption } from "../normalization-core/number-coercion.js";
