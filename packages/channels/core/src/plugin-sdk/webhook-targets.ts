// upstream: src/plugin-sdk/webhook-targets.ts@5d8067a4483
// D-CORE-321: `registerPluginHttpRoute`, `registerWebhookTargetWithPluginRoute`
// and `RegisterWebhookPluginRouteOptions` are omitted. They bind a path on
// OpenClaw's process-wide gateway HTTP server (`src/plugins/http-registry.ts`);
// Fusion has no such server, so an in-repo webhook vertical owns its own
// `node:http` listener and registers its targets in the caller-owned map that
// `registerWebhookTarget` (kept) maintains. Everything else is upstream.
