...
export async function sendGoogleChatMessage(params: {
  ...
  // D-GC-009: upstream suppresses a text follow-up that merely repeats an
  // `/approve <id> <decision>` command whose native approval card is still live
  // (`approval-card-actions.ts`). The approval-card surface is not ported —
  // the Hub owns approvals — so there is no live card to suppress against.
