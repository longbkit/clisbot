// upstream: extensions/googlechat/src/api.fetchok.transport.test.ts@5d8067a4483
...
vi.mock("./fusion/ssrf-fetch.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("./fusion/ssrf-fetch.js")>();
  return {
    ...actual,
...
describe("Google Chat real guarded transport", () => {
  ...
  // D-GC-018: the ported "delivers task-list fallback at the default chunk limit
  // through the SDK" case is omitted. It drives OpenClaw's own durable outbound
  // batch (`sendDurableMessageBatch` over `googlechatPlugin.outbound`) through
  // the plugin-runtime test registry; the Hub owns durable outbound in Fusion.
  // The chunking it asserts is covered by `format.test.ts` and the outbound
  // bridge case in `outbound.test.ts`.
