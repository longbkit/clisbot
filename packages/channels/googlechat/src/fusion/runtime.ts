// Fusion-owned runtime adapter for `extensions/googlechat/src/runtime.ts` (D-GC-002).
//
...
/** Builds the `PluginRuntime` the ported GoogleChat stores read, backed by the Hub. */
export function createGoogleChatRuntimeFromHost(host: HostRuntime): PluginRuntime {
  return {
    state: {
...
/**
 * The channel's subsystem log sink: `[googlechat/…]` lines go to the logger of
...
function installSubsystemLogSink(): void {
  installChannelSubsystemLogSink({
    channel: "googlechat",
    subsystems: ["googlechat"],
    loggers: accountLoggers,
    currentAccountId: currentGoogleChatAccountId,
  });
}
 ...
 */
export function installGoogleChatRuntime(host: HostRuntime, accountId = ""): void {
  accountLoggers.set(
    accountId,
  ...
  setGoogleChatRuntime(createGoogleChatRuntimeFromHost(host), accountId);
}
...
export function disposeGoogleChatRuntime(accountId = ""): void {
  installedHosts.delete(accountId);
  accountLoggers.delete(accountId);
  setGoogleChatRuntime(undefined, accountId);
  ...
  if (accountLoggers.size === 0) clearChannelSubsystemLogSink("googlechat");
}
