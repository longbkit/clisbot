// Fusion-owned boundary for `openclaw/plugin-sdk/ssrf-runtime` (D-GC-005).
//
...
// in, `{ response, release }` out — and the two guards the Google Chat closure
// actually depends on:
//
//   * the hostname suffix allowlist (`policy.allowedHostnameSuffixes`), which
//     is what pins Google's auth traffic to `accounts.google.com` /
//     `googleapis.com`;
//   * a private/loopback/link-local address block on the resolved hostname, so
//     a redirected or DNS-rebound Chat API host cannot reach the Hub's own
//     network.
//
...
// is accepted and ignored, which only costs the ported google-auth transport
// its proxy support.
