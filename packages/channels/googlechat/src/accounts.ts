...
function parseServiceAccount(value: unknown): Record<string, unknown> | null {
  ...
  if (typeof value === "string") {
    ...
    // D-GC-008: upstream parses with `safeParseJsonWithSchema(z.record(...))`.
    // Same contract — JSON object or null — without a zod dependency.
    try {
      return asNullableObjectRecord(JSON.parse(trimmed));
    } catch {
      return null;
    }
  }

  return asNullableObjectRecord(value);
}
...
function resolveCredentialsFromConfig(params: {
  ...
  // D-GC-008: upstream coerces the value through `coerceSecretRef`, which also
  // applies the configured secret-provider default when the value is a bare
  // string. Fusion's Hub resolves secret references before the carrier reaches
  // the vertical, so an unresolved reference is only ever an explicit one.
  if (isSecretRef(account.serviceAccount)) {
    ...
    if (resolved.status === "configured_unavailable") {
      // D-GC-008: upstream's `resolveSecretInputString` throws in `strict` mode
      // and only reports the status in `inspect` mode; core's Fusion-owned copy
      // always reports (D-CORE-257). The strict throw is restored here so a
      // drive-time resolve still fails loudly on a reference the Hub did not
      // resolve, instead of starting an account with no credential.
      if (params.mode === "strict") {
        throw new Error(
          `channels.googlechat.accounts.${accountId}.serviceAccount has an unresolved SecretRef`,
        );
      }
      return { source: "none", status: "configured_unavailable" };
    }
