...
// lifecycle hands its webhook events to. One store per channel process; the Hub
// drives one account per vertical instance today, but the map costs nothing and
// keeps multi-account starts from crossing wires. Mirrors the Discord and Telegram
// verticals' `runtime-store.ts`.
...
export function getHostRuntime(): HostRuntime {
  if (hostRuntime === undefined) {
    throw new Error("googlechat channel runtime not set (call entry.setChannelRuntime first)");
  }
  return hostRuntime;
}

/** The L3 processor for one account (created once per (runtime, account)); the
 * webhook session's admission hands its normalized events to it. */
export function registerAccountInbound(
  ...
  if (entry === undefined || entry.hostRuntime !== runtime) {
    const processor = createInboundEventProcessor({
      ...
      logger: runtime.logging.getChildLogger({ channel: "googlechat", accountId }),
    });
    entry = {
      hostRuntime: runtime,
      processor,
...
export function getAccountRuntime(accountId: string): AccountInbound {
  ...
  if (entry === undefined) {
    throw new Error(`googlechat account "${accountId}" has no inbound processor (not started?)`);
  }
  return entry;
}
